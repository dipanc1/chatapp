...
const GroupsList = () => {
    ...
    const handleAddUser = async (user1, groupId) => {
        try {
            ...
            const userIndex = await userData.groups[groupIndex].users.findIndex(
                (user) => user._id === user1,
            );
            if (userIndex === -1) {
    ...
    const fetchUsers = async () => {
        try {
            ...
            const { data } = await axios.get(
                `${backend_url}/conversation/list/5?page=${pageState.page}`,
                config,
            );
    ...
    const handleEventStatus = async (type, eventId) => {
        try {
            ...
            if (type === 'suspend') {
                setEventsBlocked((eventsBlocked) => [
                    ...eventsBlocked,
                    eventId,
                ]);
            } else if (type === 'allow') {
                setEventsAllowed((eventsAllowed) => [
                    ...eventsAllowed,
                    eventId,
                ]);
            }
        ...
        <Static>
            ...
            >
                <Heading as='h1' size='lg' fontWeight='500'>
                    All Groups {pageState?.total && <>({pageState?.total})</>}
                </Heading>
            </Flex>
            {pageState.isLoading ? (
                ...
                userData?.groups.length && (
                    <>
                        ...
                        >
                            {Array.from(
                                { length: pageState?.pages },
                                (_, index) => (
                                    <Button
                                        onClick={() => handlePage(index + 1)}
                                        background={
                                            pageState?.page === index + 1
                                                ? '#9F85F7'
                                                : '#EDF2F7'
                                        }
                                        color={
