...
const StreamingPeer = ({ setToggleChat, admin, fetchAgain, setFetchAgain }) => {
    ...
    const { selectedChat, dispatch, fullScreen, eventInfo } =
    ...
    const endStream = async () => {
        try {
            ...
            const result = await conversationApi.stopStream({ data }, config);
    ...
    const sendMeetingId = async () => {
        ...
        try {
            ...
            await conversationApi.startStream({ data }, config);
            // console.log(result);
        ...
        if (!admin) {
            try {
                const checkStream = async () => {
                    ...
                    const { data } = await conversationApi.checkStream(
                        selectedChat._id,
                        config,
                    );
                    if (!data) {
    ...
    useEffect(() => {
        ...
        const donation = async () => {
            const { data } = await donationApi.getDonationOfAnEvent(
                eventInfo.id,
                config,
            );
            if (data.length > 0) {
            ...
            {id && streamState !== null ? (
                ...
                >
                    <Box
                        ...
                        boxShadow={[
                            'unset',
                            '0px 3px 24px rgba(159, 133, 247, 0.6)',
                        ]}
                        borderRadius='5px'
                        overflow='hidden'
                        position='relative'
                                ...
                                <HStack justifyContent='center'>
                                    ...
                                    >
                                        <button onClick={shareScreen}>
                                            {screenStream?.active ? (
                                                <img
                                                    src='https://ik.imagekit.io/sahildhingra/stop-screen-share.png'
                                                    alt='share-screen'
                                                />
                                            ) : (
                                                <img
                                                    src='https://ik.imagekit.io/sahildhingra/screen-share.png'
                                                    alt='share-screen'
                                                />
                                            )}
                                        </button>
                                    ...
                                    >
                                        <button onClick={endStream}>
                                            <img
                                                src='https://ik.imagekit.io/sahildhingra/hang-up.png'
                                                alt='end'
                                            />
                                        </button>
                                    </Tooltip>
                    ...
                    <Box p={['0 20px', '0']} overflow='auto' flex='1'>
                        ...
                        <Text as='h2' size='lg' fontWeight='500' pb='15px'>
                            Host:{' '}
                            {selectedChat.groupAdmin.username.toUpperCase()}
                        </Text>
                                ...
                                <>
                                    ...
                                    >
                                        <Img
                                            ...
                                            h='20px'
                                            src='https://ik.imagekit.io/sahildhingra/dollar.png'
                                            alt=''
                                        />
                                    </Button>
                                    <Box
