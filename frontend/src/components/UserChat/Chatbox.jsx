...
export const ChatBoxComponent = ({
    ...
    const { notification, dispatch, userInfo, signature, timestamp } =
    ...
    const [loadingWhileSendingImage, setLoadingWhileSendingImage] =
        React.useState(false);
    const [loadingWhileSendingFile, setLoadingWhileSendingFile] =
        React.useState(false);
    ...
    const {
        ...
        onOpen: onOpenDocumentAttachment,
    ...
    const fetchMoreMessages = async () => {
        ...
        try {
            ...
            const { data } = await messageApi.fetchMoreMessages(
                selectedChat._id,
                page,
                config,
            );
        ...
        <>
            ...
            >
                ...
                <Input
                    ...
                    value={newMessage}
                    onKeyDownCapture={
                        newMessage !== '' ? (e) => sendMessage(e, null) : null
                    }
                />
                <Button
...
const Chatbox = ({
    ...
    const { dispatch, userInfo, fetchGroupDonations } =
        React.useContext(AppContext);
    ...
    const CheckOnlineStatus = async (friendId) => {
        try {
            ...
            const { data } = await axios.get(
                `${backend_url}/users/check-online/${friendId}`,
                config,
            );
    ...
    React.useEffect(() => {
        ...
        const donation = async () => {
            const { data } = await donationApi.getDonationOfAGroup(
                selectedChat?._id,
                config,
            );
            if (data.length > 0) {
                const peopleContributedId = new Set(
                    data.flatMap((s) =>
                        s.donatedByAndAmount.map((u) => u.user),
                    ),
                );
                const targetAmount = data.reduce(
                    (a, b) => a + b.targetAmount,
                    0,
                );
                const currentAmount = data.reduce(
                    (a, b) => a + b.currentAmount,
                    0,
                );
            ...
            {selectedChat ? (
                <>
                    ...
                    >
                        ...
                        >
                            ...
                            >
                                <Image
                                    h='18px'
                                    src='https://ik.imagekit.io/sahildhingra/back.png'
                                />
                            </Box>
                                ...
                                {selectedChat && selectedChat?.isGroupChat && (
                                    ...
                                    >
                                        <Img
                                            ...
                                            h='20px'
                                            src='https://ik.imagekit.io/sahildhingra/settings.png'
                                            alt=''
                                        />
                                    </Button>
                                )}
                    ...
                    <GroupSettingsModal
                        ...
                        setGroupChatDescription={setGroupChatDescription}
                    />
                ...
                >
                    <Image
                        src='https://cdn3d.iconscout.com/3d/free/thumb/free-chat-5658961-4715767.png'
                        h='150px'
                        mb='10px'
                    />
